      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className={`flex items-center justify-center gap-3 mb-8 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={isRTL ? 'text-right' : 'text-left'}>
            <h1 className="text-3xl font-bold text-slate-800">{t.loanTitle}</h1>
            <p className="text-slate-600">{t.loanSubtitle}</p>
          </div>
        </div>
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'} py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors`}
              ...
              <div>
                ...
                <div className="relative">
                  <Percent className={`absolute ${isRTL ? 'right-3' : 'left-3'} top-1/2 transform -translate-y-1/2 h-5 w-5 text-slate-400`} />
                  <input
                    type="number"
                    step="0.01"
              ...
              <div>
                ...
                <div className="relative">
                  <Calendar className={`absolute ${isRTL ? 'right-3' : 'left-3'} top-1/2 transform -translate-y-1/2 h-5 w-5 text-slate-400`} />
                  <input
                    type="number"
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 text-white py-4 px-6 rounded-xl font-semibold hover:from-emerald-600 hover:to-teal-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-lg hover:shadow-xl"
              >
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/50">
                    <div className={`flex items-center gap-3 mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      ...
                      <span className="text-sm font-semibold text-slate-600">{t.monthlyPayment}</span>
                    </div>
                    <p className={`text-2xl font-bold text-slate-800 ${isRTL ? 'text-right' : 'text-left'}`}>{formatCurrencyAmount(result.monthlyPayment)}</p>
                  </div>

                  <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/50">
                    <div className={`flex items-center gap-3 mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      <DollarSign className="h-5 w-5 text-blue-600" />
                      <span className="text-sm font-semibold text-slate-600">{t.totalInterest}</span>
                    </div>
                    <p className={`text-2xl font-bold text-slate-800 ${isRTL ? 'text-right' : 'text-left'}`}>{formatCurrencyAmount(result.totalInterest)}</p>
                  </div>
                </div>
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-white/50">
                  <div className={`flex items-center justify-between mb-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <h3 className={`text-xl font-bold text-slate-800 ${isRTL ? 'text-right' : 'text-left'}`}>{t.amortizationSchedule}</h3>
                    <button
                      onClick={exportToCSV}
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-200">
                          <th className={`py-3 px-2 font-semibold text-slate-600 ${isRTL ? 'text-right' : 'text-left'}`}>{t.payment}</th>
                          <th className={`py-3 px-2 font-semibold text-slate-600 ${isRTL ? 'text-left' : 'text-right'}`}>{t.principal}</th>
                          <th className={`py-3 px-2 font-semibold text-slate-600 ${isRTL ? 'text-left' : 'text-right'}`}>{t.interest}</th>
                          <th className={`py-3 px-2 font-semibold text-slate-600 ${isRTL ? 'text-left' : 'text-right'}`}>{t.balance}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {result.amortizationSchedule.slice(0, 12).map((payment) => (
                          <tr key={payment.payment} className="border-b border-slate-100 hover:bg-slate-50">
                            <td className={`py-3 px-2 text-slate-700 ${isRTL ? 'text-right' : 'text-left'}`}>{payment.payment}</td>
                            <td className={`py-3 px-2 text-slate-700 ${isRTL ? 'text-left' : 'text-right'}`}>{formatCurrencyAmount(payment.principal)}</td>
                            <td className={`py-3 px-2 text-slate-700 ${isRTL ? 'text-left' : 'text-right'}`}>{formatCurrencyAmount(payment.interest)}</td>
                            <td className={`py-3 px-2 text-slate-700 ${isRTL ? 'text-left' : 'text-right'}`}>{formatCurrencyAmount(payment.balance)}</td>
                          </tr>
                        ))}
                        {result.amortizationSchedule.length > 12 && (
                          <tr>
                            <td colSpan={4} className="py-3 px-2 text-center text-slate-500 text-sm">
